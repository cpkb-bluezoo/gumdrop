...
final class RecoverableChannelImpl implements ClientChannel {
    ...
    private static final ExchangeDeclareHandler NOOP_EXCHANGE_DECLARE =
            new ExchangeDeclareHandler() {
                @Override
                public void handleExchangeDeclareOk() {
                }
            };

    private static final QueueDeclareHandler NOOP_QUEUE_DECLARE =
            new QueueDeclareHandler() {
                @Override
                public void handleQueueDeclareOk(String queue, long messageCount,
                        long consumerCount) {
                }
            };

    private static final QueueBindHandler NOOP_QUEUE_BIND =
            new QueueBindHandler() {
                @Override
                public void handleQueueBindOk() {
                }
            };

    private static final ConfirmSelectHandler NOOP_CONFIRM_SELECT =
            new ConfirmSelectHandler() {
                @Override
    ...
    RecoverableChannelImpl(int channelId, ClientChannel live, Runnable onClosed) {
        ...
        this.internalCloseListener = new ChannelClosedListener() {
            @Override
            public void onChannelClosed(int replyCode, String replyText) {
                RecoverableChannelImpl.this.live = null;
                clearReplayState();
                if (RecoverableChannelImpl.this.onClosed != null) {
                    RecoverableChannelImpl.this.onClosed.run();
                }
                if (RecoverableChannelImpl.this.closeListener != null) {
                    RecoverableChannelImpl.this.closeListener.onChannelClosed(
                            replyCode, replyText);
                }
            }
        };
    ...
    public void queueBind(String queue, String exchange, String routingKey, FieldTable arguments,
        ...
        queueBinds.put(key, new ReplayOp() {
            @Override
            public void replay(ClientChannel liveChannel) {
                liveChannel.queueBind(queue, exchange, routingKey, arguments, NOOP_QUEUE_BIND);
            }
        });
    ...
    public void basicConsume(String queue, String consumerTag, boolean noAck, boolean exclusive,
        ...
        final String key = consumerTag.isEmpty() ? anonymousKey() : consumerTag;
        consumers.put(key, new ReplayOp() {
            ...
            public void replay(ClientChannel liveChannel) {
                ...
                liveChannel.basicConsume(queue, consumerTag, noAck, exclusive, arguments,
                        deliveryHandler, new ConsumeHandler() {
                            @Override
    ...
    public void confirmSelect(ConfirmSelectHandler handler) {
        ...
        confirmSelectOp = new ReplayOp() {
            @Override
            public void replay(ClientChannel liveChannel) {
                liveChannel.confirmSelect(NOOP_CONFIRM_SELECT);
            }
        };
