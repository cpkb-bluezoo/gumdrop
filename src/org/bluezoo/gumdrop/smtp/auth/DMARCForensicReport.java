...
public class DMARCForensicReport {
    ...
    // this class itself and is always 7-bit ASCII.
    private static final Charset ASCII = StandardCharsets.US_ASCII;

    // RFC 6591/RFC 5322: the original message or its headers (part 3)
    // may carry arbitrary 8-bit octets (e.g. an 8BITMIME message, or
    // legacy non-UTF-8 header encodings) that a caller has already
    // decoded into a String. ISO-8859-1 is the one Charset where every
    // char 0x00-0xFF maps back to the identical byte, so encoding that
    // String with it reproduces the original bytes exactly rather than
    // reinterpreting them as UTF-8 and corrupting anything outside the
    // 7-bit range.
    private static final Charset RAW_MESSAGE_CHARSET = StandardCharsets.ISO_8859_1;
    ...
    private void writeOriginalPart(WritableByteChannel out, String boundary) throws IOException {
        if (originalMessage != null) {
            ...
            write(out, CRLF, ASCII);
            write(out, originalMessage, RAW_MESSAGE_CHARSET);
            write(out, CRLF, ASCII);
            ...
            write(out, CRLF, ASCII);
            write(out, originalHeaders, RAW_MESSAGE_CHARSET);
            write(out, CRLF, ASCII);
        }
    }
