...
public class FTPDataConnectionCoordinator {
    ...
    public synchronized int setupPassiveMode(int port) throws IOException {
        ...
        try {
            ...
            passivePort = passiveConnector.getActualPort();
            if (passivePort == -1) {
                // Fallback: get port from first server channel
                java.util.List<java.nio.channels.ServerSocketChannel> channels = passiveConnector.getServerChannels();
                if (!channels.isEmpty()) {
                    java.net.InetSocketAddress addr = (java.net.InetSocketAddress) channels.get(0).getLocalAddress();
                    passivePort = addr.getPort();
                    passiveConnector.notifyBound(channels.get(0));
                }
            }
    ...
    private void establishActiveConnection() throws IOException {
        try {
            ...
            // Create data connection wrapper
            activeDataConnection = new FTPDataConnection(channel, null, false, this);
