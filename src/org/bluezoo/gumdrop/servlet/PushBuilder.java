...
class ServletPushBuilder implements PushBuilder {
    ...
    private void executePush() {
        ...
        List<Header> pushHeaders = buildPushHeaders(pushUri);
    ...
    /**
     * Build the HTTP headers list for the push request.
     */
    private List<Header> buildPushHeaders(String uri) {
        List<Header> pushHeaders = new ArrayList<>();
        
        // Add HTTP/2 pseudo-headers (required for PUSH_PROMISE)
        pushHeaders.add(new Header(":method", method));
        pushHeaders.add(new Header(":scheme", scheme));
        pushHeaders.add(new Header(":authority", authority));
        pushHeaders.add(new Header(":path", uri));
        
        // Add regular headers
        for (Map.Entry<String, List<String>> entry : headers.entrySet()) {
            String headerName = entry.getKey();
            for (String headerValue : entry.getValue()) {
                pushHeaders.add(new Header(headerName, headerValue));
            }
        }
