...
/**
 * JNI bridge to the native {@code libgumdrop} shared library.
 *
 * <p>This class provides thin wrappers around the quiche C API,
 * BoringSSL SSL_CTX management, and platform-specific utility
 * functions such as DNS nameserver discovery.
 *
 ...
 */
public final class GumdropNative {

    static {
        System.loadLibrary("gumdrop");
    }

    private GumdropNative() {
    }
    ...
    public static final int QUICHE_ERR_STREAM_RESET = -16;
    /** Connection ID limit was exceeded. */
    public static final int QUICHE_ERR_ID_LIMIT = -17;
    /** Out of available connection identifiers. */
    public static final int QUICHE_ERR_OUT_OF_IDENTIFIERS = -18;
    /** Key update error. */
    public static final int QUICHE_ERR_KEY_UPDATE = -19;
    /** Crypto buffer exceeded. */
    public static final int QUICHE_ERR_CRYPTO_BUFFER_EXCEEDED = -20;
    /** Invalid ACK range. */
    public static final int QUICHE_ERR_INVALID_ACK_RANGE = -21;
    /** Optimistic ACK attack detected. */
    public static final int QUICHE_ERR_OPTIMISTIC_ACK_DETECTED = -22;
    ...
    public static native long ssl_ctx_new(boolean isServer);

    /** Loads certificate chain (PEM) into the SSL_CTX. */
    public static native int ssl_ctx_load_cert_chain(long sslCtx,
                                                      String path);

    /** Loads private key (PEM) into the SSL_CTX. */
    public static native int ssl_ctx_load_priv_key(long sslCtx,
                                                    String path);

    /** Loads trusted CA certificates for peer verification. */
    public static native int ssl_ctx_load_verify_locations(long sslCtx,
                                                            String path);
     ...
     */
    public static native int ssl_ctx_set_ciphersuites(long sslCtx,
                                                       String ciphers);
     ...
     */
    public static native int ssl_ctx_set_groups(long sslCtx, String groups);

    /** Sets ALPN protocols on the SSL_CTX. */
    public static native int ssl_ctx_set_alpn_protos(long sslCtx,
                                                      byte[] protos);

    /** Configures whether to verify the peer's certificate. */
    public static native void ssl_ctx_set_verify_peer(long sslCtx,
                                                       boolean verify);

    /** Creates a new SSL object from the SSL_CTX (for one connection). */
    public static native long ssl_new(long sslCtx);
     ...
     */
    public static native void ssl_set_hostname(long ssl, String hostname);

    /** Frees an SSL_CTX. */
    public static native void ssl_ctx_free(long sslCtx);

    // ── quiche Config ──

    public static native long quiche_config_new(int version);

    public static native void quiche_config_set_application_protos(
            long config, byte[] protos);

    public static native void quiche_config_set_max_idle_timeout(
            long config, long ms);

    public static native void quiche_config_set_initial_max_data(
            long config, long bytes);

    public static native void quiche_config_set_initial_max_stream_data_bidi_local(
            long config, long bytes);

    public static native void quiche_config_set_initial_max_stream_data_bidi_remote(
            long config, long bytes);

    public static native void quiche_config_set_initial_max_stream_data_uni(
            long config, long bytes);

    public static native void quiche_config_set_initial_max_streams_bidi(
            long config, long count);

    public static native void quiche_config_set_initial_max_streams_uni(
            long config, long count);

    public static native void quiche_config_set_cc_algorithm(long config,
                                                              int algo);

    public static native void quiche_config_set_max_recv_udp_payload_size(
            long config, long size);

    public static native void quiche_config_set_max_send_udp_payload_size(
            long config, long size);
     ...
     */
    public static native long quiche_conn_new_with_tls(
    ...
    public static native int quiche_conn_recv(long conn, ByteBuffer buf,
                                               int len, byte[] fromAddr,
                                               byte[] toAddr);

    public static native int quiche_conn_send(long conn, ByteBuffer buf,
                                               int len);

    // ── Stream I/O (uses direct ByteBuffer) ──

    public static native int quiche_conn_stream_recv(long conn,
                                                      long streamId,
                                                      ByteBuffer buf,
                                                      int len,
                                                      boolean[] fin);

    public static native int quiche_conn_stream_send(long conn,
                                                      long streamId,
                                                      ByteBuffer buf,
                                                      int len,
                                                      boolean fin);
    ...
    public static native long[] quiche_conn_readable(long conn);

    public static native long quiche_conn_timeout_as_millis(long conn);

    public static native void quiche_conn_on_timeout(long conn);

    public static native boolean quiche_conn_is_established(long conn);

    public static native boolean quiche_conn_is_closed(long conn);

    // ── Debug logging ──

    public static native void quiche_enable_debug_logging();
     ...
     * </pre>
     */
    public static native byte[] quiche_header_info(ByteBuffer buf, int len);

    // ── Version negotiation ──

    public static native boolean quiche_version_is_supported(int version);
     ...
     */
    public static native int quiche_negotiate_version(byte[] scid,
                                                       byte[] dcid,
                                                       ByteBuffer out,
                                                       int outLen);
    ...
    public static native String ssl_get_cipher_name(long ssl);

    /** Returns the peer certificate chain in DER format. */
    public static native byte[] quiche_conn_peer_cert(long conn);
    ...
    /**
     * Returns the IP addresses of the system's configured DNS
     * nameservers as strings (e.g., "8.8.8.8", "::1").
     *
     * <p>On POSIX systems this calls {@code res_init()} and reads
     * the resolver state. On Windows this calls
     * {@code GetNetworkParams()}.
     *
     * @return array of nameserver IP address strings, or null on error
     */
    public static native String[] getSystemNameservers();
}
