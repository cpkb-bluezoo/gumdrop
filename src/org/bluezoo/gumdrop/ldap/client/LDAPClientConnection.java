...
/**
 * Event-driven, NIO-based LDAP client connection.
 *
 * <p>This connection handles the LDAP protocol using BER encoding/decoding
 * and integrates with Gumdrop's asynchronous I/O framework.</p>
 * 
 * <p>The connection implements stateful interfaces that guide the handler
...
public class LDAPClientConnection extends Connection
    ...
    /**
     * Creates an LDAP client connection.
     *
     ...
     */
    protected LDAPClientConnection(LDAPClient client, SSLEngine engine, boolean secure,
                                    LDAPConnectionReady handler) {
        super(engine, secure);
        this.client = client;
        this.handler = handler;
        this.decoder = new BERDecoder();
        this.tlsEstablished = secure;  // LDAPS is already secure
    }
    ...
    private void processMessage(ASN1Element message) throws ASN1Exception {
        if (message.getTag() != ASN1Type.SEQUENCE) {
            throw new ASN1Exception("Expected SEQUENCE, got " + ASN1Type.getTagName(message.getTag()));
        }
    ...
    private LDAPResult parseResult(ASN1Element element) throws ASN1Exception {
        ...
        return new LDAPResult(LDAPResultCode.fromCode(code), matchedDN, diagnosticMessage, referrals);
    }
    ...
    private void handleSearchResultReference(int messageId, ASN1Element element) throws ASN1Exception {
        List<ASN1Element> children = element.getChildren();
    ...
    private void handleExtendedResponse(int messageId, ASN1Element element) throws ASN1Exception {
        ...
        if (messageId == startTLSMessageId) {
            ...
            if (result.isSuccess()) {
                // Upgrade to TLS - handshakeComplete will call the callback
                try {
                    initializeSSLState();
        ...
        if (callback instanceof ExtendedResultHandler) {
            ((ExtendedResultHandler) callback).handleExtendedResult(result, responseName, responseValue, this);
        }
    }
}
