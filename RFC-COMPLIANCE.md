...
| QUIC v2 support | RFC 9369 | Compliant (without RFC 9368) | Client and server speak version 2 (`0x6b3343cf`): Initial salt, `quicv2` HKDF labels, long-header type bits and Retry Integrity Tag key/nonce come from `QuicVersion`; verified against the RFC 9369 Appendix A vectors. `QuicTransportFactory.setVersions` selects the versions and their preference order (default `1,2`). Compatible version negotiation (RFC 9368, `version_information` transport parameter) is not implemented, so a version change happens only through Version Negotiation, restarting the attempt |
