...
| Version negotiation | 6.1, 6.2, 17.2.1 | Compliant | Server answers an unsupported-version datagram of at least 1200 bytes with a Version Negotiation packet advertising every configured version (`QuicEngine.sendVersionNegotiation`, `VersionNegotiationPacket`); smaller datagrams and Version Negotiation packets are dropped without reply; responses are capped at 100 per second. Client ignores a Version Negotiation packet that does not echo both connection IDs, lists the version in use, or arrives after any other server packet; otherwise it restarts in the first configured version the server also offers (`QuicConnection.restartWithVersion`), or abandons the attempt with `QuicVersionNegotiationException` when there is none (`QuicConnection.handleVersionNegotiation`) |
| QUIC v1 support | 15 | Compliant | QUIC version 1 (RFC 9000) |
| QUIC v2 support | RFC 9369 | Compliant | Client and server speak version 2 (`0x6b3343cf`): Initial salt, `quicv2` HKDF labels, long-header type bits and Retry Integrity Tag key/nonce come from `QuicVersion`; verified against the RFC 9369 Appendix A vectors. A Retry is sent in, and accepted only in, the original version. 0-RTT stays in the original version. Session tickets are bound to the version that issued them (client cache entry, server `acceptsTicketFrom` check). `QuicTransportFactory.setVersions` selects the versions and their preference (default `1,2`). The Retry token does not encode the version (a MAY in section 4.1) |
| Compatible version negotiation | RFC 9368 | Compliant | `version_information` transport parameter (0x11) is sent, parsed and validated by both roles (`TransportParameters`, `QuicConnection.processPeerVersionInformation`). A client opens in its oldest configured version and lists the compatible ones; the server picks the client's most preferred version it accepts and switches the connection, re-deriving Initial keys and keeping the original version's Initial receive keys until a Handshake packet arrives. A client acting on Version Negotiation starts a new attempt with a fresh ClientHello (`QuicEngine.restartClientAttempt`) and validates the server's Available Versions against forged Version Negotiation. Errors use VERSION_NEGOTIATION_ERROR (0x11) and TRANSPORT_PARAMETER_ERROR. A Version Negotiation packet is not acted on if 0-RTT data was already queued on the attempt |
