 ...
 * ConnectUdpRequestHandler#headers} and, once the target resolves and
 * the policy allows it, get a real {@code 200} response back over the
 * wire -- closing the gap between {@link ConnectUdpRelayEndToEndTest}
 * (which drives {@link ConnectUdpRequestHandler} directly, proving the
 * relay logic) and the actual request-parsing/dispatch path in {@link
 * Stream#acceptConnectUdp}.
 *
 * @author <a href='mailto:dog@gnu.org'>Chris Burdess</a>
 */
public class ConnectUdpH2WireTest {
    ...
    private ByteBuffer encodeConnectUdpHeaders(String targetHost, int targetPort) throws Exception {
        ...
        request.add(new Header(":path", ConnectUdpTarget.encode(targetHost, targetPort)));
    ...
    public void testExtendedConnectRequestIsAcceptedWith200() throws Exception {
        // Real wire-level dispatch actually runs the DNS resolution (and
        // policy check) on this connection's own SelectorLoop -- but
        // headersFrameReceived() below is called directly on the test
        // thread, matching how the connection's real transport would
