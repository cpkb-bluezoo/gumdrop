...
public class MessageIndexTest {
    ...
    public void testCorruptMagicNumber() throws IOException {
        ...
        try (FileChannel ch = FileChannel.open(indexPath, StandardOpenOption.WRITE)) {
            ch.position(0);
    ...
    public void testCorruptVersion() throws IOException {
        ...
        try (FileChannel ch = FileChannel.open(indexPath, StandardOpenOption.WRITE)) {
            ch.position(4);
            ch.write(java.nio.ByteBuffer.wrap(new byte[] { (byte) 0x0F, (byte) 0x27 }));
        }
    ...
    public void testCorruptChecksum() throws IOException {
        ...
        if (fileSize > 50) {
            try (FileChannel ch = FileChannel.open(indexPath, StandardOpenOption.WRITE)) {
                ch.position(fileSize / 2);
                ch.write(java.nio.ByteBuffer.wrap(new byte[] { (byte) 0xFF }));
            }
        }
    ...
    public void testTruncatedFile() throws IOException {
        ...
        if (truncatedLen > 0) {
            Files.write(indexPath, Arrays.copyOf(bytes, truncatedLen));
        }
    ...
    public void testNonExistentFile() {
        Path nonExistent = mem.getPath("/indexes/nonexistent.gidx");
