...
public class CryptoExecutorTest {
    ...
    public void testResultDeliveredOnLoopThread() throws Exception {
        CryptoExecutor exec = new CryptoExecutor(2, 16);
        try {
            ...
            }, new CryptoExecutor.Callback<String>() {
                @Override
            ...
            assertTrue("crypto work must run on a crypto thread, was "
                    + workThread.get(),
                    workThread.get().startsWith("gumdrop-crypto-"));
        } finally {
            exec.shutdown();
        }
    }
    ...
    public void testSaturatedPoolFailsFast() throws Exception {
        ...
        CryptoExecutor exec = new CryptoExecutor(1, 1);
     ...
     * thread is still running a task, mirroring {@code StorageExecutor}'s
     * equivalent guarantee -- callers reusing process-wide state across a
     * shutdown/restart cycle (e.g. tests using
     * {@link CryptoExecutor#workThreadObserver}, or Gumdrop.shutdown()
     * followed by Gumdrop.start()) need "shutdown() returned" to actually
    ...
    public void testShutdownWaitsForInFlightTaskToFinish() throws Exception {
        ...
        exec.submit(endpoint, new Callable<Void>() {
            ...
            public Void call() {
                ...
                // Simulate CPU-bound work that doesn't check interrupted
                // status (a real delegated task's crypto math wouldn't
                // either): spin for a fixed duration, so shutdownNow()'s
                // interrupt alone cannot make this task stop any sooner.
    ...
    private static final class NoopCallback<T>
            implements CryptoExecutor.Callback<T> {
