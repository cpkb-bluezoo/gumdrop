/*
 * MailboxWatcherIntegrationTest.java
...
/**
 * Integration tests for {@link MailboxWatcher} against the real file system's
 * {@code WatchService}. Event latency is platform dependent (about 2 seconds
 * per event on macOS), and the tests do real disk I/O, so they live here;
 * dispatch logic is covered deterministically by {@code MailboxWatcherTest}.
 *
 * @author <a href='mailto:dog@gnu.org'>Chris Burdess</a>
 */
public class MailboxWatcherIntegrationTest {
    ...
    public void register_notifiesOnMatchingFileCreated() throws Exception {
        ...
        assertTrue("no event within 30s", notified.await(30, TimeUnit.SECONDS));
    ...
    public void register_ignoresNonMatchingFile() throws Exception {
        ...
        assertTrue("no event within 30s", canaryNotified.await(30, TimeUnit.SECONDS));
    ...
    public void register_multipleListenersOnSameDirectoryAllNotified() throws Exception {
        ...
        assertTrue("no event within 30s", first.await(30, TimeUnit.SECONDS));
        assertTrue("no event within 30s", second.await(30, TimeUnit.SECONDS));
    }
